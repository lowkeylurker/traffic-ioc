\section{Kiến trúc tổng quan của Data Pipeline}
\subsection{Sơ lược hệ thống ETL trong đồ án}
...
\subsection{Quản lý ngân sách gọi API và tính bền vững}
...
\subsection{Tính lũy đẳng (Idempotency) và cơ chế xử lý xung đột dữ liệu}
...
\subsection{Cụ thể hóa trong các Pipeline nghiệp vụ}
...
\subsection{Kiến trúc hệ thống và điều phối}
...
\subsubsection{Luồng thực thi tuần tự của chu kỳ ETL (Sequence Flow).}
\begin{center}
...
\subsubsection{Triển khai cấp độ điều phối}
...
\subsubsection{Ràng buộc tài nguyên trong Data Ingestion}
...
\subsubsection{Xây dựng kế hoạch "Budget-Safe Realtime" và lấy mẫu có trọng số}
\subsubsubsection{Phân bổ ngân sách gọi API động (Dynamic Budget Allocation)}
...
\subsection{Cơ chế quản lý khóa API động và cân bằng tải}
\subsubsection{Nhận thức trạng thái}
...
\subsubsection{Mẫu thiết kế Circuit Breaker và Failover tự động}
...
\subsection{Sức bền mạng và khả năng chịu lỗi}
\subsubsection{Cơ chế Retry with Exponential Backoff}
...
\subsubsection{Xử lý phân cực: lỗi tạm thời với lỗi hệ thống}
...
\subsection{Đa dạng hóa nguồn thu thập và xử lý bất đồng bộ}
\subsubsection{Thu thập dữ liệu tĩnh (Topology) vs. Động (Time-series)}
...
\subsection{Trình tự nạp dữ liệu và ràng buộc}
\subsubsection{Nguyên lý ràng buộc toàn vẹn tham chiếu và cấu trúc DAG}
...
\subsubsection{Pha 1: Khởi tạo dữ liệu nền tảng}
...
\subsubsection{Pha 2: Xây dựng Topology Không gian}
...
\subsubsection{Pha 3: Nạp dữ liệu chuỗi thời gian}
...
\subsubsection{Pha 4: Hậu xử lý và dữ liệu phái sinh}
