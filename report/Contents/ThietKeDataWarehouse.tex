\section{1. Kiến trúc tổng quan của Data Pipeline}
\subsection{1.1. Sơ lược hệ thống ETL trong đồ án}
...
\subsection{1.2. Xây dựng và thiết kế hệ thống ETL trong đồ án}
...
\subsubsection{1.2.1. BaseExtractor: Chuẩn hóa giao thức mạng và cơ chế chịu lỗi}
...
\subsubsection{1.2.2. BaseTransformer: Đảm bảo tính Toàn vẹn Dữ liệu bằng Pure Functions}
...
\subsubsection{1.2.3. BaseLoader: Trừu tượng hóa Chiến lược Nạp dữ liệu vào Data Warehouse}
...
\subsection{1.3. Quản lý ngân sách gọi API và tính bền vững}
...
\subsection{1.4. Tính lũy đẳng (Idempotency) và cơ chế xử lý xung đột dữ liệu}
...
\subsection{1.5. Cụ thể hóa trong các Pipeline nghiệp vụ}
...
\subsection{1.6. Kiến trúc hệ thống và điều phối}
\subsubsection{1.6.1 Sơ đồ kiến trúc:}
\subsubsection{}
\begin{center}
...
\subsubsection{1.6.2 Luồng thực thi tuần tự của chu kỳ ETL (Sequence Flow).}
\begin{center}
...
\subsubsection{1.6.3 Triển khai cấp độ điều phối}
...
\subsection{2.1. Ràng buộc tài nguyên trong Data Ingestion}
...
\subsection{2.2. Xây dựng kế hoạch "Budget-Safe Realtime" và lấy mẫu có trọng số}
\subsubsection{2.2.1. Phân bổ ngân sách gọi API động (Dynamic Budget Allocation)}
...
\subsubsection{2.2.2. Kỹ thuật lựa chọn ưu tiên: Gold Corridors và Critical Score}
...
\subsection{2.3. Cơ chế quản lý khóa API động và cân bằng tải}
\subsubsection{2.3.1. Nhận thức trạng thái}
...
\subsubsection{2.3.2. Mẫu thiết kế Circuit Breaker và Failover tự động}
...
\subsection{2.4. Sức bền mạng và khả năng chịu lỗi}
\subsubsection{2.4.1. Cơ chế Retry with Exponential Backoff}
...
\subsubsection{2.4.2. Xử lý phân cực: lỗi tạm thời với lỗi hệ thống}
...
\subsection{2.5. Đa dạng hóa nguồn thu thập và xử lý bất đồng bộ}
\subsubsection{2.5.1. Thu thập dữ liệu tĩnh (Topology) vs. Động (Time-series)}
...
\subsubsection{2.5.2. Mở rộng kích thước lưới cho dữ liệu bổ trợ}
...
\section{3. Trình tự nạp dữ liệu và ràng buộc}
\subsection{3.1. Nguyên lý ràng buộc toàn vẹn tham chiếu và cấu trúc DAG}
...
\subsection{3.2. Pha 1: Khởi tạo dữ liệu nền tảng}
...
Hệ quy chiếu Thời gian (Temporal Dimensions): Dữ liệu được nạp theo chuỗi phụ thuộc tuyến tính: dim\_month\_year  dim\_date và dim\_shift  dim\_time\_of\_day. Đây là các bảng tĩnh, thường được nạp sẵn dữ liệu (pre-populated) cho nhiều năm, làm nền tảng phân hoạch (partitioning) cho toàn bộ Data Warehouse.


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image9.png}
\end{center}
...
\subsection{3.3. Pha 2: Xây dựng Topology Không gian}
...
\subsection{3.4. Pha 3: Nạp dữ liệu chuỗi thời gian}
...
\subsection{3.5. Pha 4: Hậu xử lý và dữ liệu phái sinh}
...
\section{4. Logic Biến đổi Dữ liệu}
\subsection{4.1. Nguyên lý hàm thuần túy trong tầng biến đổi}
...
\subsection{4.2. Khai phá và biến đổi động lực học dòng xe}
...
\subsubsection{4.2.1. Chuỗi biến đổi chỉ số cơ bản (Traffic Index, Delay \& LOS)}
Hệ thống sử dụng các phép toán vô hướng để thiết lập các chỉ số cơ bản:


Tỷ lệ vận tốc (Speed Ratio - r): Được định nghĩa là tỷ số giữa vận tốc thực tế (v) và vận tốc thiết kế/thông thoáng ().


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image18.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image6.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image5.png}
\end{center}

Độ trễ hành trình (Delay Seconds - ): Là phần thời gian chênh lệch mà phương tiện phải đánh đổi do sự suy giảm tốc độ.


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image19.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image14.png}
\end{center}

Phân cấp Mức độ Phục vụ (Level of Service - LOS): Dữ liệu định lượng (I) được rời rạc hóa (discretization) thành các nhãn định tính (từ A đến F) thông qua hệ luật chuyên gia (Rule-based mapping), lưu vào cột los\_level. Ví dụ: LOS A biểu thị dòng xe di chuyển tự do (), trong khi LOS F biểu thị trạng thái vỡ trận/tắc nghẽn nghiêm trọng (). Bước này rất quan trọng để hỗ trợ hệ thống Dashboard hiển thị màu cảnh báo tự động.


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image1.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image22.png}
\end{center}

\subsubsection{4.2.2. Bài toán Ước lượng Lưu lượng bằng Nghịch đảo Hàm BPR}
...
Hàm BPR nguyên thủy mô tả sự gia tăng thời gian di chuyển dựa trên tỷ số giữa lưu lượng và công suất ():
...
\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image8.png}
\end{center}
...
: Thời gian hành trình thông thoáng (Free-flow Travel Time).


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image26.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image15.png}
\end{center}

: Các hệ số hiệu chuẩn (thường mặc định là ).


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image2.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image13.png}
\end{center}

Vì dữ liệu thu thập được là vận tốc, hệ thống thực hiện xấp xỉ  và nghịch đảo hàm BPR để tìm ra lưu lượng (q):


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image24.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image12.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image28.png}
\end{center}

Trong đó,  (max\_vc\_ratio) là ngưỡng chặn trên (thường là 1.0 hoặc 1.2). Kỹ thuật này ngăn chặn hiện tượng tràn số (Overflow) hoặc rác dữ liệu sinh ra khi vận tốc v tiến dần về 0 trong các tình huống kẹt xe nghiêm trọng.
...
\subsection{4.3. Xử lý không gian và thuật toán khớp bản đồ}
...
\subsubsection{4.3.1. Truy vấn K-Nearest Neighbors (KNN) với PostGIS}
...
Về mặt độ phức tạp thuật toán, toán tử <-> khai thác triệt để cấu trúc cây R-Tree của chỉ mục không gian (GiST Index) idx\_dim\_segment\_center\_gist. Nó so sánh khoảng cách giữa tọa độ sự cố với các hộp giới hạn (Bounding Boxes) của đoạn đường, giúp giảm độ phức tạp tìm kiếm từ  (Full Table Scan) xuống mức , đáp ứng hoàn hảo yêu cầu xử lý thời gian thực.


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image4.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image11.png}
\end{center}

\subsubsection{4.3.2. Tiền xử lý hình học đa dạng (Geometry Preprocessing)}
...
\subsection{4.4. Chuẩn hóa ngữ nghĩa và phân rã chiều thời gian}
...
time\_key (Số phút tích lũy trong ngày): Được tính bằng công thức  (thuộc miền giá trị ). Việc ánh xạ sang dim\_time\_of\_day với độ phân giải tính bằng phút giúp Data Warehouse dễ dàng thực hiện các phép Roll-up (tổng hợp dữ liệu) theo khung 5 phút, 15 phút hoặc theo Ca trực (Shift) mà không cần dùng đến các hàm xử lý chuỗi thời gian nặng nề.


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image21.png}
\end{center}

\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image25.png}
\end{center}

Chuẩn hóa Mức độ nghiêm trọng (Severity): Mức độ sự cố từ nhà cung cấp được ánh xạ tịnh tiến vào miền giá trị  thông qua hàm normalize\_magnitude(), đảm bảo sự đồng nhất ngữ nghĩa khi truy vấn dữ liệu từ nhiều nguồn khác nhau (như TomTom, OpenWeather, hay mạng xã hội).


\begin{center}
\includegraphics[width=0.85\linewidth]{Images/image16.png}
\end{center}

\subsection{4.5. Kết luận}
...
\section{5. Kỹ thuật tối ưu hóa kho dữ liệu}
\subsection{5.1. Mục tiêu tối ưu hóa}
...
\subsection{5.2. Chiến lược phân mảnh dữ liệu chuỗi thời gian}
...
\begin{lstlisting}[language=SQL]
SQL-- Trích xuất từ DDL khai báo bảng fact_traffic_flowCREATE TABLE public.fact_traffic_flow (    traffic_flow_key bigint NOT NULL,    segment_key bigint NOT NULL,    time_key integer NOT NULL,    date_key integer NOT NULL,    -- ... các trường dữ liệu ...) PARTITION BY RANGE (date_key);-- Khởi tạo phân vùng vật lý riêng biệt cho tháng 03/2026CREATE TABLE public.fact_traffic_flow_202603 ( ... );ALTER TABLE ONLY public.fact_traffic_flow ATTACH PARTITION public.fact_traffic_flow_202603 FOR VALUES FROM (20260301) TO (20260401);
\end{lstlisting}
...
\subsection{5.3. Chiến lược đánh chỉ mục đa hình}
...
\subsubsection{5.3.1. Chỉ mục không gian GiST (Generalized Search Tree)}
...
\begin{lstlisting}[language=SQL]
SQLCREATE INDEX idx_fact_incident_geom_gist ON ONLY public.fact_incident USING gist (geometry);CREATE INDEX idx_dim_segment_center_gist ON public.dim_segment USING gist (geometry_center);
\end{lstlisting}
...
\subsubsection{5.3.2. Chỉ mục khối dải BRIN (Block Range Index)}
...
\begin{lstlisting}[language=SQL]
SQLCREATE INDEX idx_fact_traffic_flow_ts_brin ON ONLY public.fact_traffic_flow USING brin ("timestamp") WITH (pages_per_range='32');
\end{lstlisting}
...
\subsubsection{5.3.3. Chỉ mục một phần (Partial Index) hỗ trợ Alerting}
...
\begin{lstlisting}[language=SQL]
SQL-- Chỉ tạo index cho các sự cố có mức độ nghiêm trọng (severity_level) >= 4CREATE INDEX idx_fact_incident_severe ON ONLY public.fact_incident USING btree (severity_level, segment_key) WHERE (severity_level >= 4);-- Chỉ tạo index cho các đoạn đường bị ùn tắc nặng (LOS = E hoặc F)CREATE INDEX idx_fact_flow_bad_los ON ONLY public.fact_traffic_flow USING btree (los_level, segment_key) WHERE (los_level = ANY (ARRAY['E'::bpchar, 'F'::bpchar]));
\end{lstlisting}
...
\subsection{5.4. Cơ chế UPSERT và Quản lý "Bloat" trong MVCC}
...
\begin{lstlisting}[language=SQL]
SQLALTER TABLE ONLY public.fact_traffic_flow_202603    ADD CONSTRAINT fact_traffic_flow_202603_pkey PRIMARY KEY (traffic_flow_key, date_key);
\end{lstlisting}
