 ...
 */
router.get('/vehicle-mix', authMiddleware, (req, res, next) => analyticsController.getVehicleMix(req, res, next));

/**
 * GET /api/v1/analytics/speed-comparison
 * Lấy dữ liệu so sánh tốc độ hiện tại vs baseline
 */
router.get('/speed-comparison', authMiddleware, (req, res, next) =>
 ...
 */
router.get('/reliability-ranking', authMiddleware, (req, res, next) =>
