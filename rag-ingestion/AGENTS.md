...
### 3. Static Type Checking with Pyrefly
> [!IMPORTANT]
> **Pyrefly Type Check Rule**:
> Verify static typing across all Python modules using Pyrefly:
> ```bash
> cd rag-ingestion
> uv run pyrefly check
> ```

### 4. Object Storage Reference (MinIO / S3)
> [!IMPORTANT]
...
- **`src/services/`**:
  ...
  - `embedder.py`: Extensible `EmbedderFactory` wrapping `BaseEmbedder` and `OpenAIEmbedder` via OpenAI SDK.
  - `qdrant_sync.py`: Qdrant collection upsertion with full metadata payload indexing.
  - `oltp_sync.py`: PostgreSQL OLTP metadata synchronization using SQLAlchemy ORM models.
- **`src/parsers/`**: Modular document extractors:
  ...
  - `ocr_vision_parser.py`: Google Gemini 1.5 Flash Vision OCR for scanned decree PDFs using `google-genai`.
