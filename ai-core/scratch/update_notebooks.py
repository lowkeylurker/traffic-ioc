...
def update_notebook_01():
    path = Path('notebooks/01_Data_Extraction_Feature_Engineering.ipynb')
    if not path.exists():
    ...
    for cell in nb['cells']:
        if cell['cell_type'] == 'code':
            ...
            if "cols_to_plot = [" in source and "speed_ratio" in source:
                # Target: "cols_to_plot = ['current_speed_kmh', 'traffic_index', 'delay_seconds', 'speed_ratio']"
                new_source = source.replace(", 'speed_ratio'", "").replace("'speed_ratio', ", "").replace("'speed_ratio'", "")
                cell['source'] = [line + ("\n" if not line.endswith("\n") else "") for line in new_source.splitlines()]
                updated = True
                print(f"Updated cols_to_plot in {path}")
    ...
    for cell in nb['cells']:
        if cell.get('id') == 'eda_refinement_header':
            new_source = [
                ...
                "- `current_speed_kmh`, `delay_seconds`: Các biến động lực học chính.\n",
