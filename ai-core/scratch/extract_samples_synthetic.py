...
# Đọc dữ liệu đã cân bằng (chứa dữ liệu nhân tạo)
data_path = "/workspace/ai-core/data/processed/02_balanced_training_data.parquet"
print(f"Loading {data_path}...")
df = pd.read_parquet(data_path)

def extract_synthetic_samples():
    print("=== SAMPLES NHÂN TẠO (AUGMENTED) CLASS 4 & 5 ===")
    
    # Lọc dữ liệu Synthetic
    df_synthetic = df[df['synthetic_flag'] == 1].copy()
    
    # Lấy nhãn cuối cùng của mỗi segment (target)
    last_rows = df_synthetic.groupby('segment_key').tail(1)
    
    for cls in [4, 5]:
        print(f"\n{'='*80}")
        print(f" TÌM KIẾM MẪU CLASS {cls} (AUGMENTED)")
        print(f"{'='*80}")
        ...
        if len(target_segs) == 0:
            print(f"Không tìm thấy mẫu Augmented nào cho Class {cls}.")
            continue
        ...
        for i, seg in enumerate(sample_segs):
            ...
            seq = df_synthetic[df_synthetic['segment_key'] == seg].copy()
            
            # Tính lại speed_ratio nếu cần
