...
import { MOCK_ALERTS, DEFAULT_MAP_CENTER, DEFAULT_MAP_ZOOM } from '@/config/constants'
...
export const TrafficLayerPage: React.FC = () => {
    ...
    const { data: trafficData = FALLBACK_DATA, isLoading: mapLoading, error: apiError } = useQuery({
    ...
    const error = apiError ? (apiError instanceof Error ? apiError.message : 'Failed to fetch') : globalError
    ...
    const heatmapLayerStyle = useMemo(() => ({
        ...
        type: 'heatmap',
        paint: {
            'heatmap-weight': ['interpolate', ['linear'], ['get', 'avgSpeed'], 0, 0, 70, 1],
            'heatmap-intensity': ['interpolate', ['linear'], ['zoom'], 0, 1, 18, 3],
            'heatmap-color': ['interpolate', ['linear'], ['heatmap-density'], 0, '#008000', 0.33, '#ffff00', 0.66, '#ff7f00', 1, '#ff0000'],
            'heatmap-radius': ['interpolate', ['linear'], ['zoom'], 0, 2, 18, 20],
            'heatmap-opacity': ['interpolate', ['linear'], ['zoom'], 0, 0.8, 18, 0.3],
        },
    }) as LayerProps, [])
    ...
    const handleCompassReset = () => mapRef.current?.easeTo({ bearing: 0, pitch: 0, duration: 500 })

    return (
        <div ref={containerRef} style={{ position: 'relative', height: '100%', width: '100%', overflow: 'hidden', borderRadius: 8, boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.03), 0 1px 6px -1px rgba(0, 0, 0, 0.02), 0 2px 4px 0 rgba(0, 0, 0, 0.02)' }}>
            {(isLoading || mapLoading) && trafficData.features.length === 0 && (
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 10, backgroundColor: 'white', padding: '20px', borderRadius: '4px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
                    <Spin tip="Loading traffic map..." />
                </div>
            )}

            {error && trafficData.features.length === 0 && (
                <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 10, backgroundColor: '#ff4d4f', color: 'white', padding: '12px 16px', borderRadius: '4px', fontSize: '14px' }}>
            ...
            <Map
                ref={mapRef}
                initialViewState={{ longitude: DEFAULT_MAP_CENTER[0], latitude: DEFAULT_MAP_CENTER[1], zoom: DEFAULT_MAP_ZOOM }}
            ...
            <MapControls onZoomIn={handleZoomIn} onZoomOut={handleZoomOut} onCompass={handleCompassReset} onCamera={() => setCCTVModalVisible(true)} onHeatmapToggle={setHeatmapEnabled} />
            <MapLegend />
            <CCTVModal visible={cctvModalVisible} onClose={() => setCCTVModalVisible(false)} />
        </div>
    )
}
