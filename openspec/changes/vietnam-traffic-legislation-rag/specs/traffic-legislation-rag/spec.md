...
- **THEN** the system SHALL resolve dependencies via `uv` using `pyproject.toml`, follow a clean layered architecture separating centralized core infrastructure (`src/core/config.py`, `src/core/db.py`, `src/core/celery_app.py`), Celery task workers (`src/tasks/ingestion_tasks.py`), declarative ORM models (`src/models/oltp.py`), HTTP controllers (`api/routes/ingest.py`), DTO schemas (`schemas/ingest.py`), and background pipeline orchestrator (`services/ingestion_pipeline.py`), maintain a reproducible `uv.lock`, and execute distributed tasks via Celery worker `uv run celery -A src.core.celery_app.celery_app worker` and FastAPI server `uv run uvicorn src.main:app --host 0.0.0.0 --port 8001 --reload` using modern lifespan context manager.
