...
Commands:
    ...
    run-mock-traffic     Bơm dữ liệu giả lập vào fact_traffic_flow
...
@app.command("run-mock-traffic")
def run_mock_traffic() -> None:
    """Script bơm dữ liệu giả lập vận tốc liên tục vào bảng fact_traffic_flow."""
    from src.mockers.traffic_simulator import run
    
    console.print(Panel.fit(
        "[bold cyan]🚗 MOCK TRAFFIC FLOW[/bold cyan]\n"
        "[dim]Giả lập vận tốc xe cộ theo thời gian thực...[/dim]",
        border_style="cyan"
    ))
